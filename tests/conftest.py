...
def mock_bot():
    ...
    bot.cumulative_price = AsyncMock(
        return_value=CumulativePrices(Offers=[], Targets=[], UpdatedAt=0)
    )
...
def make_market_offer(
    title="Test Skin", game_id="rust", price_usd=150, item_id="item1", exterior=None
):
...
def make_skin_history(title="Test Skin", game="rust", avg_price=150, count=20):
    ...
    return SkinHistory(title=title, game=game, sales=sales, avg_price=avg_price, update_time=now)


def make_aggregated_price(
    title="Test Skin", order_best=1.40, order_count=5, offer_best=1.60, offer_count=10
):
    return AggregatedPrice(
        title=title,
        orderBestPrice=order_best,
        orderCount=order_count,
        offerBestPrice=offer_best,
