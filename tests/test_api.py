...
from api.schemas import (
    Games, Balance, LastSales, MarketOffers, AggregatedPrice,
    AggregatedPricesResponse, UserTargets, ClosedTargets, Target,
    CreateTargets, CreateTarget, LastPrice, CumulativePrices,
    UserItems, ClosedOffers, CreateOffers, CreateOffer,
    CreateOffersResponse, EditOffers, EditOffer, EditOffersResponse,
    DeleteOffers,
)
from api.exceptions import (
    BadRequestError, BadGatewayError, TooManyRequests,
    BadAPIKeyException, WrongResponseException,
)
...
class TestAggregatedPricesEndpoint:
    ...
    async def test_uses_post_method(self):
        api = DMarketApi('pub_key', '0' * 128)
        api.api_call = AsyncMock(return_value={
            'aggregatedPrices': [
                {'title': 'Skin A', 'orderBestPrice': 1.5, 'orderCount': 3,
                 'offerBestPrice': 1.8, 'offerCount': 10}
            ],
            'nextCursor': None
        })

        result = await api.aggregated_prices(['Skin A'])
    ...
    async def test_sends_correct_body(self):
        api = DMarketApi('pub_key', '0' * 128)
        api.api_call = AsyncMock(return_value={
            'aggregatedPrices': [], 'nextCursor': None
        })
    ...
    async def test_returns_aggregated_prices(self):
        ...
        api.api_call = AsyncMock(return_value={
            'aggregatedPrices': [
                {'title': 'A', 'orderBestPrice': 1.0, 'orderCount': 1,
                 'offerBestPrice': 2.0, 'offerCount': 5}
            ],
            'nextCursor': None
        })
...
class TestDeleteTarget:
    ...
    async def test_batches_over_150(self):
        ...
        targets = [Target(
            TargetID=f't{i}', Title='Test', Amount='1', Status='Active',
            GameID=Games.RUST, Attributes=[], Price=LastPrice(Currency='USD', Amount=1.0)
        ) for i in range(200)]
    ...
    async def test_sends_model_dump(self):
        ...
        body = CreateTargets(
            ...
            Targets=[CreateTarget(
                Amount='1',
                Price=LastPrice(Currency='USD', Amount=1.5),
                Title='Test Skin'
            )]
        )
